...
Table.prototype.whereClause = function(where, values, offset) {
	if(where && where.length > 0) {
		var i = offset || 1;
		return (
			' WHERE ' +
				where.map(function(propertyName) {
					if(values && (i - 1) < values.length && values[i - 1] === null) {
						values.splice(i - 1, 1);
						return '"' + inflection.underscore(propertyName) + '" IS NULL';
					}
					else {
...
Table.prototype.selectStatement = function(where, limit, sort, values) {
	return (
		'SELECT * FROM ' +
		'"' + this.name + '"' +
...
Table.prototype.updateStatement = function(set, where, limit, values) {
	return (
		'UPDATE "' + this.name + '"' +
		this.setClause(set) +
		this.whereClause(where, values, set.length + 1) +
		' RETURNING *'
	);
};
...
Table.prototype.deleteStatement = function(where, values) {
	return (
		'DELETE FROM "' + this.name + '" ' + this.whereClause(where, values)
	);
};
...
Table.prototype.update = function(setKeys, whereKeys, limit, values) {
	return this.query(this.updateStatement(setKeys, whereKeys, limit, values), values);
};

Table.prototype.select = function(whereKeys, limit, values, sort) {
	return this.query(this.selectStatement(whereKeys, limit, sort, values), values);
};

Table.prototype.remove = function(whereKeys, values) {
	return this.query(this.deleteStatement(whereKeys, values), values);
};
