	...
	Object.keys(properties).forEach(function(propertyName) {
		...
		if(property.isAllowed()) {
			if(parentAssociation) {
				columns.push(referenceTableName + '.' + property.columnName + ' AS ' + property.columnNameIndicator + parentAssociation.name + property.columnNameIndicator + property.columnNameIndicator + referenceProperty.name + property.columnNameIndicator + property.columnName);
			}
			else {
...
Table.prototype.createSelectStatement = function(whereMap, limit, sort) {
	...
	function parseAssociation(association, parentAssociation) {
		...
		if(parentAssociation) {
			targetTableName = parentAssociation.getAssociatedModel().getTable().getName();
		}
		else {
		...
		if(association.isManyToMany()) {
			...
			return 'LEFT OUTER JOIN ' + associatedTable.getName() + ' ON (' + associatedTable.getName() + '.' + relation1Property.columnName + ' = ' + targetTableName + '.id)';
		}
		else {
			...
			if(association.options.hasOne && (association.options.autoFetch || whereMap[association.name])) {
				...
				return self.getJoinType(association) + ' ' + referenceTableName + ' ON (' + targetTableName + '.id = ' + referenceTableName + '.' + associationProperty.columnName + ')';
			}
				...
				return self.getJoinType(association) + ' ' + referenceTableName + ' ON (' + targetTableName + '.' + association.columnName + ' = ' + referenceTableName + '.id)';
			}
