...
Table.propertyTypes = {
	...
	Many: function(modelNameOrModel) {
		if(!modelNameOrModel) {
			throw new Error('No model or model name in Many(). Can you try re-ordering the creation of your models or are you referencing an invalid model?');
		}
		...
		return function(property) {
			...
			// Set reference to the current model e.g. Projects
			property.referenceName = modelName;

			// Let's find the associated model
			var associatedModel = property.getReference();

			// and add a reference of this model to the associated model
			var properties = {};
			var associationName = inflection.camelize(property.model.getName(), true);
			properties[associationName] = [property.model.Reference(property.model)];
			associatedModel.addProperties(properties, false);

			property.manyAssociation = associationName;
		};
	},
...
Table.prototype.alter = function(addedProperties, removedProperties) {
	console.log('alter: ' + this.alterStatement(addedProperties, removedProperties));
...
Table.prototype.createSelectStatement = function(whereMap, limit, sort) {
	...
	result.query += Object.keys(this.associations).map(function(associationName) {
		...
		if(association.oneAssociation) {
			// TODO: this should be a LEFT OUTER JOIN and INNER JOIN if required only
			return ' INNER JOIN ' + referenceTableName + ' ON (' + tableName + '.' + association.columnName + ' = ' + referenceTableName + '.id)';
		}
		else if(association.manyAssociation) {
			// TODO: do an INNER JOIN if the property is required
			var associationProperty = association.getReference().getProperty(association.manyAssociation);

			return ' LEFT OUTER JOIN ' + referenceTableName + ' ON (' + tableName + '.' + 'id' + ' = ' + referenceTableName + '.' + associationProperty.columnName + ')';
		}
		else {
