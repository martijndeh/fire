	...
	Many: function(modelNameOrModel) {
		...
		if(modelNameOrModel) {
			return function(property) {
				...
				properties[associationName] = [property.model.HasOne(property.model)];
	...
	Reference: function(modelNameOrModel) {
		if(modelNameOrModel) {
			return function(property) {
				...
				if(reference) {
					return 'INTEGER REFERENCES ' + reference.getTable().getName() + '(id)';
				}
				else {
					return 'INTEGER REFERENCES ' + inflection.tableize(property.referenceName) + '(id)';;
				}
			};
		}
