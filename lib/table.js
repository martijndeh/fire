		...
		if(modelNameOrModel) {
			return function(property) {
				...
				if(reference) {
					return {
						index: 999,
						clause: 'REFERENCES ' + reference.getTable().getName() + '(id)'
					};
				}
				else {
					return {
						index: 999,
						clause: 'REFERENCES ' + inflection.tableize(property.referenceName) + '(id)'
					};
				}
			};
		}
		...
		if(modelNameOrModel) {
			return function(property) {
				...
				if(property.required) {
					props.push(property.model.Required);
				}

				props.push(property.model.HasOne(property.model));

				properties[associationName] = props;
