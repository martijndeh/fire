...
Migration.prototype.removeProperty = function(property) {
    ...
    if(!this.findTaskByName('destroyModel', property.model)) {
        ...
        console.log('Removing property, but there is a destroy model task...');
    }
};
...
Migration.prototype.sortTasks = function() {
    ...
    while(this.tasks.length) {
        ...
        if((task.name != 'createModel' || Object.keys(associations).every(function(propertyName) {
    ...
    this.tasks = this.tasks.sort(function(a, b) {
        var numberOfAssociationsA = Object.keys(a.model.getAssociations()).length;
        if(numberOfAssociationsA == 0) {
            console.log(a.model.getName() + ' has no associations');
            return 1;
        }

        var numberOfAssociationsB = Object.keys(b.model.getAssociations()).length;
        if(numberOfAssociationsB == 0) {
            console.log(b.model.getName() + ' has no associations');
            return -1;
        }

        var properties = a.params.addedProperties || a.model.getAssociations();

        var exists = Object.keys(properties).some(function(propertyName) {
            var property = properties[propertyName];
            return (property.options.belongsTo && property.options.referenceName == b.model.getName());
        });

        if(exists) {
            console.log(a.model.getName() + ' belongs to ' + b.model.getName());

            // So, A belongs to B
            return 1;
        }

        properties = b.params.addedProperties || b.model.getAssociations();

        exists = Object.keys(properties).some(function(propertyName) {
            var property = properties[propertyName];
            return (property.options.belongsTo && property.options.referenceName == a.model.getName());
        });

        if(exists) {
            console.log(b.model.getName() + ' belongs to ' + a.model.getName());
        ...
        // TODO: prefer edit, create, destroy models ...

        
        

        console.log(a.model.getName() + ' (' + numberOfAssociationsA + ') ' + ' is the same as ' + b.model.getName() + ' (' + numberOfAssociationsB + ')');
        return 0;
    });
    */
}
