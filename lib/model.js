...
Model.prototype.removeManyAssociationTo = function(model) {
	...
	Object.keys(this._associations).forEach(function(associationName) {
		...
		if(association.manyAssociation && association.getReference() == model) {
...
Model.prototype.removeOneAssociationTo = function(model) {
	...
	Object.keys(this._associations).forEach(function(associationName) {
		...
		if(association.oneAssociation && association.getReference() == model) {
			self.removeProperty(association);
		}
	});
};
...
Model.prototype.removeProperty = function(property) {
	...
	if(property.isAssociation()) {
		...
		if(property.manyAssociation) {
			property.getReference().removeOneAssociationTo(this);
		}
		else if(property.oneAssociation) {
			// TODO: this isn't really safe... there could be more associations really
			property.getReference().removeManyAssociationTo(this);
		}	
	}
};
...
Model.prototype.find = function(whereMap, sort) {
	if(this._activeMigration) {
		console.log('Active migration & find!');
		console.log('Queing ' + utils.getMethodName(arguments.callee, Model.prototype));
...
Model.prototype.destroy = function(cascade) {
	// Let's check if it's really set to true instead of truthy as cascading might be dangerous
	// A falsey value is fine, as it is not dangerous
	if(typeof cascade != 'undefined' && (cascade != false) && (cascade !== true)) {
		throw new Error('In Model#destroy cascade argument is set to `' + cascade + '` (a truthy) and not either `true` or `false`.');
	}

	if(cascade) {
		console.log('WARNING: Use Model#forceDestroy instead of Model#destroy(true) to drop cascade tables.');
	}
	...
	return this._table.drop(cascade);
};
