...
function App(name, settings, container) {
	...
	for(var i = 0, il = this._settings.modules.length; i < il; i++) {
		...
		if(moduleName == 'ngRoute') {
			console.log('Node on Fire switched to CommonJS loading on the client-side. This move deprecates bower in favor of npm (which is already a dependency).');
			console.log('You have `ngRoute` configured as a dependency. Please replace this with `angular-route` instead.');

			throw new Error('`ngRoute` found instead of `angular-route`.');
		}
	}
...
App.prototype._start = function() {
	...
	debug('App#_start (arguments: ' + Object.keys(argv).join(', ') + ')');
