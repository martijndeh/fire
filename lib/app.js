...
App.prototype.createController = function(route) {
	var controllerClass = require(route.caller.fullPath);
	var controller = new controllerClass();
	controller._clone(route.caller);
	return route;
}
...
App.prototype.parseConnection = function(connection) {
	...
	if(!route) {
		...
		this.responders.parseConnection(connection)
			...
			.fail(function(error) {
				connection.sendError(error || new Error(404))
			});
	}
	else {
		...
		this.middlewares.parseConnection(connection, controller)
			.then(function() {
				return Q(route.method.apply(controller, Array.prototype.slice.call(route.matches, 1)));
			})
			.then(function(response) {
				return this.middlewares.sendConnection(connection, controller)
					.then(function() {
						return Q(response);
					});
			}.bind(this))
