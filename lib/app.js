	...
	for(var i = 0, il = this._settings.modules.length; i < il; i++) {
		...
		if(moduleName == 'ngRoute') {
			throw new Error([
				...
				'',
				'Lastly, update your `view.jade` to remove all unnecessary scripts and load the `/scripts/bundle.min.js.`. This files includes all JavaScript.'
			].join('\n'));
		}
	}
	...
	return (function setupModules(app) {
		...
		app.modules.forEach(function(module_) {
			result = result.then(function() {
				...
				if(module_.setup) {
					return Q.when(app.injector.call(module_.setup, {basePath: config.basePath}, module_));
				}
			});
		});
		...
		.then(function startModules() {
			...
			self.modules.forEach(function(module_) {
				if(module_.start) {
					startingModules.push(self.injector.call(module_.start, {}, module_));
				}
			});
