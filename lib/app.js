...
/**
 * Sets up a model. This is a convenient method. For a full explanation see {@link Models#model}.
 *
 * @param  {Constructor} modelConstructor The model to be created.
...
/**
 * Sets up a worker. This is a convenient method. See {@link Workers#worker}.
 *
 * @param  {Constructor} workerConstructor The worker to create.
 * @return {App}                   This app instance.
 */
App.prototype.worker = function(workerConstructor) {
	this.workers.worker(workerConstructor);
	return this;
};
...
App.prototype.stop = function() {
	...
	// TODO: Should we send a stop to every module?

	var self = this;
	return Q.when(this.workers && this.workers.stop())
		.then(function() {
			var defer = Q.defer();

			if(self.server) {
				self.server.close(defer.makeNodeResolver());
			}
			else {
				defer.resolve();
			}
	...
	return (function _setup(app) {
		...
		.then(function setupServer() {
			...
			if(argv.worker) {
				...
				return self.workers.startConsuming(workerNames);
			}
			else {
				var port = null;
