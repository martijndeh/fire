...
var RouterHTTP = require('./modules/router-http');
var RouterWebSocket = require('./modules/router-web-socket');
...
var WebSockets = require('./modules/web-sockets');
...
function App(name, settings, container) {
	...
	if(typeof this._settings.modules == 'undefined') {
		debug('Warning: no modules defined. Please specify one or more modules in your app.');
...
App.prototype._loadModules = function() {
	...
	this.addModule(Router);
	this.addModule(WebSockets);
	this.addModule(RouterHTTP);
	this.addModule(RouterWebSocket);
...
App.prototype.addModule = function(moduleConstructor) {
	...
	Object.defineProperty(this, propertyName, {
		get: function() {
			//console.log('Warning: `app.' + propertyName + '` is deprecated. Please use dependency injection instead.');
	...
	this.injector.register(propertyName, module_);
...
App.prototype.stop = function() {
	...
	this.modules.forEach(function(module_) {
		if(module_.stop) {
			debug('Queueing module `' + module_.constructor.name + '` to stop');
