...
function Controller(fullPath, models) {
	...
	this.name 	= utils.normalizeClassName(this.constructor).toLowerCase(); //path.basename(fullPath, path.extname(fullPath));
...
Controller.prototype.clone = function(controller) {
...
Controller.prototype.createPath = function(method) {
...
Controller.prototype.createViewPath = function(methodName) {
...
Controller.prototype.createRoutes = function() {
	var routes = [];

	for(var methodName in this) {
		if(methodName.length && !Controller.prototype[methodName] && typeof this[methodName] == 'function') {
			routes.push(new Route(this.createPath(this[methodName]), this.createViewPath(methodName), this, this[methodName], utils.captureOne(methodName, /^([a-z]+)/), this.filter));
		}
	}
...
Controller.prototype.render = null;
