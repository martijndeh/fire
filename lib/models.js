...
Models.prototype.setup = function(path) {
	...
	for(var modelName in this.internals) {
		...
		this.addModel(ModelClass, modelName, Model);
	}
...
Models.prototype.addModel = function(ModelClass, modelName, Model) {
	...
	Model.call(model, this);

	// This also needs to get passed to the model instances
	// Perhaps we should make a generic system
	model.workers = this._workers;
