...
Controllers.prototype.setup = function(basePath) {
	...
	if(this.app.HTTPServer && this.app.HTTPServer.express) {
		...
		this.app.HTTPServer.express.use(bodyParser.urlencoded({
			extended: true
		}));
		this.app.HTTPServer.express.use(bodyParser.json());
...
Controllers.prototype.loadControllerConstructor = function(controllerPath, controllerConstructor, fullPath) {
	...
	// TODO: deprecate the view-based routes, which is the old syntax to declare routes. The new syntax is defined in the middleware module.
...
Controllers.prototype.addViewRoute = function(controllerPaths, controllerConstructor) {
	...
	if(!page.template) {
		if(self.app.container.numberOfApps() > 1) {
			page.template = '.fire/.build/public/' + self.app.name + '/templates/view.html';
		}
		else {
			page.template = '.fire/.build/public/templates/view.html';
		}
	}
...
Controllers.prototype.addRoute = function(route) {
	...
	if(route.isView) {
		...
		this.addViewRoute([{path: route.path}], route.controllerConstructor);
	}
	else {
				...
				.then(function() {
					return route.method.apply(controller, route.argumentNames.map(function(argumentName) {
						if(argumentName[0] == '$' && argumentName.length > 1) {
							...
							return request.params[argumentName];
						}
					}));
				})
