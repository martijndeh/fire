...
function Controllers(app) {
	...
	app.controller = function(controllerPath, controllerConstructor) {
		// In angular we cannot specify a path per controller. Instead, routes are generated based on view-methods on the controller's prototype.
		// In ractive we can specify a path per controller.
		if(self.app.type == 'angular') {
			self.controller(null, controllerPath);
		}
		else if(self.app.type == 'ractive') {
			...
			throw new Error('In App#controller unknown app type `' + self.app.type + '`.');
		}
...
Controllers.prototype.addViewRoute = function(controllerPath, controllerConstructor) {
	...
	if(self.app._options.stylesheets) {
	...
	this.app.HTTPServer.express.get(controllerPath, function parseView(request, response) {
		var options = {
			...
			styles: self.app._options.styles || page.styles || [],
			scripts: self.app._options.scripts || page.scripts || []
		};
