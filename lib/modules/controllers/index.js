...
Controllers.prototype.setup = function(basePath) {
	...
	utils.readDir(path.join(basePath, 'controllers'), function(filePath) {
		// We load the file. In the files controller get added if they call app.controller().
		var controllerConstructor = require(filePath);

		// but we do not expect anything on `exports = module.exports = ...` as nowadays
		// users should use app.controller(...) to define controllers.
