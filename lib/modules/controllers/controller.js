...
function Controller(models, request, response) {
	...
	if((request.headers['x-json-params'] === 'true' || request.headers['x-json-params'] == '1')) {
		...
		this.body = parseJSONParams(request.body);
		this.query = parseJSONParams(request.query);
	}
	else {
...
Controller.prototype.findAuthenticator = function() {
	...
	if(!authenticatorModel || !this.session.at) {
		return Q.when(null);
	}
