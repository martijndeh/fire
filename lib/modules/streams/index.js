	...
	models.forEach(function(model) {
		if(!model.isPrivate) {
			...
			models.sql([
				...
				'		PERFORM pg_notify(\'' + channelName + '\', json_build_object(\'type\', TG_OP, \'row\', row_to_json(NEW))::text);',
				'		RETURN NEW;',
				'	ELSE',
				'		PERFORM pg_notify(\'' + channelName + '\', json_build_object(\'type\', TG_OP, \'row\', row_to_json(OLD))::text);',
				'		RETURN OLD;',
				'	END IF;',
				'END;',
				'$$ LANGUAGE plpgsql;',
