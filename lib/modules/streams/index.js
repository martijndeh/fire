...
function Streams(HTTPServer, webSockets, models) {
		...
		models.forEach(function(model) {
			...
			if(model && !model.isPrivate && model.getName) {
				...
				models.sql([
					...
					'        SELECT INTO payload json_build_object(\'type\', TG_OP, \'row\', row_to_json(NEW), \'old\', row_to_json(OLD))::text;',
	...
	var _registerStream = function(stream, connection) {
		...
		if(!isListening) {
			return _listeners.add(stream.modelName, function(payloadMap) {
				debug('Parse `' + payloadMap.type + '`');

				// TODO: We should select streams based on an index. Now we're iterating over all streams for a given model. This can be optimized to include the whereMap.

				if(payloadMap.type == 'INSERT') {
					return _parseAdded(stream.modelName, payloadMap.row);
				}
				else if(payloadMap.type == 'UPDATE') {
					return _parseChanged(stream.modelName, payloadMap.row, payloadMap.old);
				}
				else if(payloadMap.type == 'DELETE') {
					return _parseRemoved(stream.modelName, payloadMap.row);
				}
				else {
	...
	var _parseSubscribe = function(connection, messageMap) {
		...
		return _canSubscribe(stream)
			...
			.then(function(modelInstances) {
				...
				return _sendModelInstances(stream, 'added', modelInstances);
			})
	...
	var _sendModelInstances = function(stream, type, modelInstances) {
	...
	var _sendRemovedModelInstance = function(stream, modelInstance) {
		if(_match(stream, modelInstance)) {
			return _sendModelInstances(stream, 'removed', [
				modelInstance
			]);
		}
	};

	var _sendChangedModelInstance = function(stream, modelInstance, oldModelInstance) {
		var newMatch = _match(stream, modelInstance);
		var oldMatch = _match(stream, oldModelInstance);

		debug('_sendChangedModelInstance ' + newMatch + ' ' + oldMatch);

		if(newMatch && oldMatch) {
			return _sendModelInstances(stream, 'changed', [
				modelInstance
			]);
		}
		else if(newMatch && !oldMatch) {
			return _sendModelInstances(stream, 'added', [
				modelInstance
			]);
		}
		else if(!newMatch && oldMatch) {
	...
	var _sendAddedModelInstance = function(stream, modelInstance) {
		if(_match(stream, modelInstance)) {
			return _sendModelInstances(stream, 'added', [
				modelInstance
			]);
	...
	var _parseChanged = function(modelName, rowMap, oldMap) {
		var streams = _streams[modelName] || [];
		if(streams.length) {
			return _loadModelInstance(modelName, rowMap)
				.then(function(modelInstance) {
					return _loadModelInstance(modelName, oldMap)
						.then(function(oldModelInstance) {
							var result = Q.when(true);

							streams.forEach(function(stream) {
								result = result.then(function() {
									return _sendChangedModelInstance(stream, modelInstance, oldModelInstance);
								});
							});
	...
	var _parseRemoved = function(modelName, rowMap) {
		var streams = _streams[modelName] || [];
		if(streams.length) {
			return _loadModelInstance(modelName, rowMap)
				.then(function(modelInstance) {
					...
					streams.forEach(function(stream) {
						result = result.then(function() {
							return _sendRemovedModelInstance(stream, modelInstance);
						});
					});
	...
	var _parseAdded = function(modelName, rowMap) {
		debug('Streams#parseAdded');
		...
		if(streams.length) {
			return _loadModelInstance(modelName, rowMap)
				.then(function(modelInstance) {
					...
					streams.forEach(function(stream) {
						result = result.then(function() {
							return _sendAddedModelInstance(stream, modelInstance);
						});
					});
