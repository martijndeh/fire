...
app.factory('FireStreamInstance', ['FireUUID', function(FireUUID) {
    return function(model) {
        ...
        var _emit = function(event, parameter) {
            _eventListenersMap[event].forEach(function(callback) {
                callback(parameter);
            });
        };

        this.on = function(event, callback) {
            if(typeof _eventListenerMap[event] != 'undefined') {
                var eventListeners = _eventListenerMap[event];
        ...
        this.parseMessage = function(messageMap) {
            if(messageMap.msg == 'added') {
                var modelInstances = model.parseResult(messageMap.result);

                this.results = this.results.concat(modelInstances);
                modelInstances.forEach(function(modelInstance) {
                    _emit('added', modelInstance);
                });
                ...
                var modelInstances = model.parseResult(messageMap.result);
                for(var i = 0, il = modelInstances.length; i < il; i++) {
                    var modelInstance = modelInstances[i];

                    for(var j = 0, jl = this.results.length; j < jl; j++) {
                        var existingModelInstance = stream.results[j];

                        if(modelInstance.id == existingModelInstance.id) {
                            existingModelInstance.refresh(modelInstance);
                            _emit('changed', existingModelInstance);
                            break;
                        }
                    }
                }
                ...
                for(var i = 0, il = modelInstances.length; i < il; i++) {
                    ...
                    for(var j = 0, jl = this.results.length; j < jl; j++) {
                        ...
                        if(modelInstance.id == existingModelInstance.id) {
                            ...
                            _emit('removed', existingModelInstance);
                            break;
                        }
                    }
                }
...
app.service('FireStreamService', ['FireSocketService', 'FireUUID', function(FireSocketService, FireUUID) {
    ...
    function parseMessage(messageMap) {
        ...
        if(stream) {
            stream.parseMessage(messageMap);
        }
    }
    var close = null;
    ...
    this.open = function(model, whereMap, optionsMap) {
        ...
        var stream = new FireStreamInstance(model);
...
app.service('FireModelInstance', ['$injector', '$q', function($injector, $q) {
        ...
        if(typeof modelInstance._map[propertyName] != 'undefined' && modelInstance._map[propertyName] !== null) {
            if(Array.isArray(modelInstance._map[propertyName])) {
                ...
                if(modelInstance._map[propertyName]._map) {
                    modelInstance._map[propertyName] = new fireModelInstanceConstructor(modelInstance._map[propertyName]._map, modelInstance._path + '/' + '{{resourceName}}');
                }
                else {
                    modelInstance._map[propertyName] = new fireModelInstanceConstructor(modelInstance._map[propertyName], modelInstance._path + '/' + '{{resourceName}}');
                }
            }
        }
    }
    ...
    this.createXToManyMethods = function(modelInstance, modelName, propertyName, resource, singularMethodName, pluralMethodName) {
        ...
        modelInstance['remove' + singularMethodName] = function(modelInstanceOrUUID) {
            ...
            return $injector.get(modelName + 'Model')._action('delete', modelInstance._model.endpoint + '/' + this.id + '/{{resource}}/' + UUID)
                .then(function(removedModelInstance) {
...
app.factory('FireModelInstance{{name}}', [{{modelInstanceDependencyNames}}, function({{modelInstanceDependencies}}) {
    return function(setMap, path, shouldBeUndefined) {
        FireModelInstance.construct(this, setMap, path, {{name}}Model);

        var self = this;
    {{#properties}}
    	{{#isAssociation}}
    	FireModelInstance.parseAssociation(this, '{{name}}', '{{resourceName}}', '{{getAssociatedModelName}}');
    	{{/isAssociation}}
...
{{#methods}}
        {{#isOneToOne}}
        FireModelInstance.createOneToOneMethods(this, '{{modelName}}', '{{propertyName}}', '{{resource}}', '{{singularMethodName}}');
        {{/isOneToOne}}
        {{#isXToMany}}
        FireModelInstance.createXToManyMethods(this, '{{modelName}}', '{{propertyName}}', '{{resource}}', '{{singularMethodName}}', '{{pluralMethodName}}');
        {{/isXToMany}}
{{/methods}}
    };
}]);
