 ...
 * 	return new this.app.bridge.Generator(path.join(__dirname, 'templates', this.app.type, 'my-module.js'), {
 * 		// Module data here.
 * 	});
 * };
 * ```
...
function Bridge(app) {
	...
	var disabled = this.app._options.disabled;

	this.configMap = (require('./' + app.type));

	if(!this.configMap) {
		throw new Error('Invalid app type `' + app.type + '`. There is no Bridge module available for this type. The following types are valid: `angular`, `ractive`.');
	}
	...
	this.configMap.methodNames.forEach(function(methodName) {
		...
		if(disabled) {
			if(app[methodName]) {
				...
				console.log('Warning: Bridge wants to swizzle method App#' + methodName + ' but it already exists.');
			}
