...
function Bridge(app) {
	...
	if(!this.configMap) {
		throw new Error('Invalid app type `' + app.type + '`. There is no Bridge module available for this type.');
	}
	...
	this.configMap.methodNames.forEach(function(methodName) {
		...
		if(disabled) {
			if(app[methodName]) {
				...
				console.log('Warning: Bridge wants to swizzle method App#' + methodName + ' .');
			}
