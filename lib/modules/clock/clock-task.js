...
function ClockTask(timingPattern, name, callback) {
	this._name = name;
	this._job = new cron.CronJob(timingPattern, this.run.bind(this), null, false, process.env.TZ || 'UTC');
		...
		.then(function(clockTaskResult) {
			if(clockTaskResult) {
				...
				if(nextDate < currentDate) {
					debug('Cron task skipped scheduled time interval. Running immediately now.');

					return self.run();
				}
				else {
					debug('Cron task not skipped scheduled time interval. Not running immediately now.');
				}
			}
			else {
				return self._createTaskResult();
