...
Generate.prototype.unswizzleMethods = function() {
	debug('Generate#unswizzleMethods');
...
Generate.prototype.createMigrations = function(fromBuild) {
	...
	function sqlMigrationTask(sql) {
		debug('sqlMigrationTask `' + sql + '`');

		return '\tthis.models.sql([\n' + sql.split('\n').map(function(line) {
	...
	return oldModels.setup(null)
		...
		.then(function() {
			...
			Object.keys(newModels._sql).forEach(function(hash) {
				var newSql = newModels._sql[hash];
				var oldSql = oldModels._sql[hash];

				if(!self.app.settings('isMaster') && self.app.container.numberOfApps() > 1) {
					debug('Not adding sql in non-master app.');
				}
				else if(!oldSql) {
					upMigrationTasks.push(sqlMigrationTask(newSql));
				}
			});
