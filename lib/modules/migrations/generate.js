...
function propertyTypesToString(property) {
	console.log('Checking property ' + property.name);
	...
	return '[' + property.types.map(function(type) {
		...
		if(type.params && type.params.length > 0 && type.params[0] != property) {
			propertyTypeString += '(' + type.params.map(function(value, index) {
				...
				if(!name) {
					throw new Error('Invalid or unknown model name in property model `' + type.name + '`.');
				}
...
function Generate(startPath) {
	if(startPath) {
		this.path = path.resolve(startPath);
	}
	else {
		this.path = null;
	}
...
Generate.prototype.createMigration = function() {
