 ...
 * @constructor
 */
function Migration(version, checksum, models) {
...
Migration.prototype.createEditTask = function(existingModel) {
    ...
    debug('Add task `' + 'editModel' + '` (`' + existingModel.getName() + '`).');
...
Migration.prototype.destroyModel = function(existingModel) {
    debug('Migration#destroyModel `' + existingModel.getName() + '`');
...
Migration.prototype.go = function(direction) {
    ...
    // TODO: Use the injector.
    return Q.when(this[direction].call(this))
        .then(function() {
            if(!self.tasks.length) {
                console.log('Warning: there are only 0 tasks in your migration. Continuing anyway.');
            }
            else {
        ...
        .then(function() {
            var defer = Q.defer();
            var appName = self.models.app.name.replace(/-new$/, '');

            if(direction == 'up') {
                // We migrated upwards, so we insert a schema with our version
                var createSchema = function(count) {
                    var defer2 = Q.defer();

                    self.models.execute('INSERT INTO schemas (version, checksum, app) VALUES (?, ?, ?)', [self.version, self.checksum, appName])
                        .then(function() {
                            defer2.resolve();
                        })
                        .catch(function(error) {
                            if(error.code == '42P01') {
                                if(count > 1) {
                                    ...
                                    createSchema(count + 1)
                                        .then(defer.resolve)
                                        .catch(defer.reject)
                                        .done();
                                }
                            }
                            else if(error.message == 'column "checksum" of relation "schemas" does not exist' || error.message == 'column "app" of relation "schemas" does not exist') {
                                self.models.execute('INSERT INTO schemas (version) VALUES (?)', [self.version])
                                    .then(function() {
                                        defer2.resolve();
                                    })
                                    .catch(function(error2) {
                                        defer2.reject(error2);
                                    })
                                    .done();
                            }
                            else {
                ...
                self.models.execute('DELETE FROM schemas WHERE version = ? AND app = ?', [self.version, appName])
                    .then(function() {
                        defer.resolve();
                    })
                    .catch(function(error) {
                        // If this is an undefined table error, the schemas table is destroyed which means we're at version 0.
                        if(error.code == '42P01') {
                            defer.resolve();
                        }
                        else if(error.message == 'column "checksum" of relation "schemas" does not exist' || error.message == 'column "app" of relation "schemas" does not exist' || error.message == 'column "app" does not exist' || error.message == 'column "checksum" does not exist') {
                            self.models.execute('DELETE FROM schemas WHERE version = ?', [self.version])
                                .then(function() {
                                    defer.resolve();
                                })
                                .catch(function(error2) {
...
Migration.prototype.soft = function() {
    debug('Migration#soft (' + this.tasks.length + ' tasks)');

    // We want to make sure forward-references work during our soft migrations as well
    this.activate();

    var self = this;
    return Q.when(self.models.app.injector.call(self.up, {}, self))
        .then(function() {
            // Migration tasks created: let's see if we have any pending model creations
            var modelNames = [];
            self.tasks.forEach(function(task) {
            ...
            return Q.when(self.models.app.injector.call(self.down, {}, self))
                .then(function() {
                    // Now we remove all tasks
                    self.tasks = [];
                    ...
                    return self.models.app.injector.call(self.up, {}, self);
                })
                .then(function() {
        ...
        .finally(function() {
            self.models.isSoftMigrating = false;
        });
};
