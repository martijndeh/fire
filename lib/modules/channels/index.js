...
Channels.prototype.stages = ['build', 'release', 'run'];
...
Channels.prototype.generator = function() {
	return new this.app.bridge.Generator(path.join(__dirname, 'templates', this.app.settings('type'), 'channel-services.js'), {channels: Object.keys(this._constructors).map(function(name) {
		return {name: name};
	})});
};
...
Channels.prototype.setup = function(basePath) {
	if(basePath) {
		...
		this.app.requireDirSync(path.join(basePath, 'channels'));
	}
...
Channels.prototype.parsePacket = function(packet, webSocket) {
	return this.getChannel(packet.channel.type, packet.channel.id)
