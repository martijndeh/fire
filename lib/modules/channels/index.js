...
Channels.prototype.generator = function() {
	return new this.app.bridge.Generator(path.join(__dirname, 'channel-services-js.mu'), {channels: Object.keys(this._constructors).map(function(name) {
		return {name: name};
	})});
};
...
Channels.prototype.addChannelConstructor = function(channelConstructor) {
	var channelName = channelConstructor.name;
	if(this._constructors[channelName]) {
		throw new Error('Channel `' + channelName + '` already exists.');
	}

	debug('Create channel ' + channelName);

	util.inherits(channelConstructor, Channel);

	this._constructors[channelName] = channelConstructor;

	var channelType = channelName;
	var self = this;
	this[channelType] = {
