...
* ```
*
* Also a clock process is created. This is the process which times tasks and schedules work at the right time. The clock process is managed by the {@link Clock} module.
* ```
...
Schedulers.prototype.stages = ['build', 'release', 'run'];
...
Schedulers.prototype.setup = function(basePath) {
	...
	if(basePath) {
		...
		this.app.requireDirSync(path.join(basePath, 'schedulers'));
	}
...
Schedulers.prototype.numberOfSchedulers = function() {
	if(this._constructors) {
		return Object.keys(this._constructors).length;
	}

	return Object.keys(this._schedulersMap).length;
};
...
Schedulers.prototype.addSchedulerConstructor = function(schedulerConstructor) {
	...
	var scheduler = this.app.injector.construct(schedulerConstructor);
