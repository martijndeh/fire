 ...
 * ```
 *
 * Triggers internally use the {@link Clock} module. This means you must declare the clock process type must be declared in your `Procfile`. Node on Fire should automatically generate this for you.
 *
 * @param {App} app
 * @constructor
 */
...
Triggers.prototype.migrate = function(models) {
	...
	if(authenticator) {
		authenticator._addProperty('triggerResult', [authenticator.Where('NOT EXISTS (SELECT * FROM trigger_results WHERE trigger_results.trigger_name = $1 AND trigger_results.subject = ' + authenticator.getTable().name + '.id)')]);
	}
};
...
Triggers.prototype.startIntervals = function() {
	...
	Object.keys(this._triggersMap).forEach(function(triggerName) {
		...
		self.app.clock.addTask(trigger.timingPattern, trigger.timeZone, trigger.name, function() {
