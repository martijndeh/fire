			...
			return function(property) {
				...
				if(associatedModel) {
					...
					if(associatedProperty) {
						if(associatedProperty.options.hasMany) {
							// We do not allow automatic many-to-many model relations anymore. Instead, users need to configure an inbetween model manually.
							throw new Error('Automatic many-to-many relations are deprecated. Please declare a connecting model manually and create two one-to-many relations.');
						}
