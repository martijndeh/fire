...
exports = module.exports = {
	...
	Authenticate: function() {
		return function(property) {
			...
			model.addProperty(new Property('password', [model.String, model.Required, model.Hash(function(value) {
				var hash = crypto.createHash('sha512');
				hash.update(value);
				return hash.digest('hex');
			})]));
			
			model.addProperty(new Property('accessToken', [model.String, model.Default(function() {
				var defer = Q.defer();
	...
	HasMany: function(modelNameOrModel, linkedPropertyName) {
		...
		if(modelNameOrModel) {
			return function(property) {
				...
				console.log(property.model._name + '.' + property.name + ' HasMany to ' + property.options.referenceName);
				...
				if(associatedModel) {
					...
					if(associatedProperty) {
						if(associatedProperty.options.hasMany) {
							...
							console.log('Creating many-to-many via ' + name);
