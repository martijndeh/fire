...
var propertyTypes = {
	...
	*/
	...
	/**
	 * Creates a virtual read-only property.
	 *
	 * This allows you to create a static value, or calculated value based on other properties, without actually storing it.
	 *
	 * `readOnly` should be an SQLish string, see {@link Table#parseSQLishStatement} for more information on SQLish.
	 *
	 * For example:
	 * ```
	 * function Book() {
	 * 	this.title = [this.String];
	 * 	this.titleLength = [this.ReadOnly('LENGTH($title) * 2')];
	 * }
	 * ```
	 * This creates a book model with a `title` property and a `titleLength` property which returns the length of the title times two.
	 *
	 ...
	 * ```
	 *
	 * In the future this method will be extended so you can also provide a function to return a read-only value.
	 *
	 * @param {String} readOnly SQLish statement.
	 * @name PropertyTypes#ReadOnly
	 * @function
	 */
	 ...
	 * })
	 * .then(function(test) {
	 * 	// test.three is not set via the transform method (thus it's the default value)
	 * })
	 * ```
	 *
	...
	Transform: function(method) {
		return function(property) {
			...
			for(var i = 0, il = property.options.transformKeyNames.length; i < il; i++) {
				...
				// TODO: What if a property already exists?
	...
	Has: function(hasMethod, shouldBeUndefined) {
		if(typeof shouldBeUndefined != 'undefined') {
			throw new Error('PropertyTypes#Has expects only one argument. `hasModel` is deprecated.');
		}
				...
				if(associatedModel) {
					...
					if(associatedProperty) {
						if(associatedProperty.options.hasMany) {
							...
							property.options.throughPropertyName 			= utils.lcfirst(propertyNames[0]);
							associatedProperty.options.throughPropertyName 	= utils.lcfirst(propertyNames[1]);
							...
							var properties = {};
							properties.id = [];
							properties[property.options.throughPropertyName] = [property.model.BelongsTo(property.model)];
							properties[associatedProperty.options.throughPropertyName] = [associatedModel.BelongsTo(associatedModel)];
	 ...
	 * ```
	 *
	 * @param {String} propertyName The name of the property belonging to a one-to-many or many-to-many association.
	 *
	...
	Aggregate: function(aggregateFunctionName, propertyName) {
		return function(property) {
			...
			property.options.readOnly = aggregateFunctionName.toUpperCase() + '(' + targetProperty.model.getTable().getName() + '.' + targetProperty.columnName + ')';
		};
	},
	...
	Authenticate: function() {
		return function(property) {
			...
			var resetPasswordModel = model.models.createModel(model.getName() + 'ResetPassword', {
				authenticator: [model.BelongsTo(model), model.Required],
			...
			model.addProperty(new Property('resetPassword', [model.HasOne(resetPasswordModel)], model, model.models), true);
		};
	},
