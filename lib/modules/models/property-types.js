		...
		if(modelNameOrModel) {
			return function(property) {
				...
				if(options && typeof options != 'object') {
					throw new Error('`options` in HasMany must be an object.');
				}
					...
					if(associatedProperty) {
						if(associatedProperty.options.hasMany) {
							...
							if(!throughModel) {
								...
								throw new Error('In a many-to-many association, please specify the through model manually. In a previous version `' + name + '` would be created automatically, but not anymore.');
							}
