...
Table.prototype.createWhereClause = function(where, offset) {
	...
	if(propertyNames.length > 0) {
		...
		result.clause += propertyNames.map(function(propertyName) {
			...
			if(firstProperty) {
				valueResult = self.createValueClause(property.columnName, whereValue, i, firstProperty);
				...
				valueResult = self.createValueClause(columnName, whereValue, i, property);
				i += valueResult.values.length;
...
Table.prototype.createValueClause = function(columnName, whereValue, i, property) {
	var result = {
		clause: '',
		values: []
	};
	...
	if(property && property.options.rawWhere) {
		...
		result.clause = property.options.rawWhere.replace('$1', '$' + (i++));
	}
...
Table.prototype.createDeleteStatement = function(whereMap, limit, skip, sort) {
	...
	// TODO: Is limit, skip, sort applied correctly when deleting?
