...
Table.prototype.execute = function(sql, values) {
	return this.query(this.parseSQLishStatement(sql), values);
};
		...
		result.clause += propertyNames.map(function(propertyName) {
			...
			if(firstProperty) {
				...
				if(firstProperty.isManyToMany()) {
					var otherProperty = firstProperty.options.through.getProperty(firstProperty.options.relationshipVia.options.throughPropertyName);

					return 'EXISTS (SELECT * FROM ' + property.model.getTable().getName() + ' WHERE ' + valueResult.clause + ' AND ' + otherProperty.columnName + ' = ' + self.getName() + '.id)';
				}
				else {
...
Table.prototype.selectClauseForAssociation = function(referenceProperty, parentAssociation, select) {
...
Table.prototype.createSelectStatement = function(whereMap, limit, skip, sort, select, fetchAssociations) {
	var result = {
		query: '',
		values: []
	};
	...
	this.forEachProperty(function(property) {
		if(!selectIsArray || property.name == 'id' || select.indexOf(property.name) >= 0) {
			if(property.isAllowed()) {
	...
	this.forEachAssociation(function(association) {
		if(association.options.autoFetch || fetchAssociations && fetchAssociations.indexOf(association.name) >= 0) {
			columns.push(self.selectClauseForAssociation(association, null, select));
	...
	Object.keys(childAssociations).forEach(function(parentName) {
		...
		childAssociations[parentName].forEach(function(association) {
			columns.push(self.selectClauseForAssociation(association, parentAssociation, select));
		});
	});
...
Table.prototype.select = function(whereMap, limit, skip, sort, select, associations) {
	var result = this.createSelectStatement(whereMap, limit, skip, sort, select, associations);
