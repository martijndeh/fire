...
Table.prototype.getProperty = function(propertyName) {
	...
	if(!property) {
		// TODO: This is also in Model#getProperty. We should use only that one instead.

		var propertyNames = propertyName.split('.', 2);
		if(propertyNames.length > 1) {
			var firstProperty = this.properties[propertyNames[0]];
...
Table.prototype.getFirstProperty = function(keyPath) {
	...
	if(propertyNames.length > 1) {
		return this.properties[propertyNames[0]];
	}
	return null;
};
		...
		result.clause += propertyNames.map(function(propertyName) {
			...
			if(property.isManyToMany()) {
				...
				if(firstProperty) {
					...
					columnName = property.model.getTable().getName() + '."' + property.columnName + '"';
				}
			}
...
Table.prototype.shouldJoinAssociation = function(association, whereMap) {
	var self = this;
	return (whereMap[association.name] || Object.keys(whereMap).some(function(propertyName) {
		var propertyNames = propertyName.split('.', 2);
		var firstProperty = null;

		if(propertyNames.length > 1) {
			firstProperty = self.properties[propertyNames[0]];
		}

		return (firstProperty == association);
	}));
};
...
Table.prototype.createSelectStatement = function(whereMap, limit, sort) {
	...
	Object.keys(this.associations).forEach(function(associationName) {
		...
		var joinStatement = self.createJoinStatement(association);
		if(association.options.autoFetch) {
			joinStatements.push(joinStatement);
		}

		if(self.shouldJoinAssociation(association, whereMap)) {
			whereJoinStatements.push(joinStatement)
		}
	});
	...
	Object.keys(childAssociations).forEach(function(parentName) {
		...
		childAssociations[parentName].forEach(function(association) {
			var joinStatement = self.createJoinStatement(association, parentAssociation);
			if(association.options.autoFetch) {
				joinStatements.push(joinStatement);
			}
