...
Table.prototype.addJoinStatement = function(query, associations) {
	...
	if(association.isManyToMany()) {
		...
		query = query.leftOuterJoin(associatedTable.name, associatedTable.name + '.' + relation1Property.columnName, previousAlias + '.id');


		query = query.leftOuterJoin(association.getAssociatedModel().getTable().name + ' as ' + alias, associatedTable.name + '.' + relation2Property.columnName, alias + '.id');
	}
	else {
		...
		if(!selectIsArray || selectAllProperties || propertyName == 'id' || selectProperties.indexOf(propertyName) >= 0) {
			if(property.isAllowed()) {
				...
				if(property.options.sqlish && typeof property.options.sqlish == 'string') {
					...
					query = query.column(self.knex.raw(sqlish).wrap('(', ') as ' + associations.map(function(association) {
						return association.name + property.columnNameIndicator;
					}).join('') + property.columnName));
				}
			}
		...
		if(sort) {
			...
			Object.keys(sort).forEach(function(propertyName) {
				...
				if(property.options.sqlish && typeof property.options.sqlish == 'string') {
					...
					withQuery = withQuery.column(self.knex.raw(sqlish).wrap('(', ') as ' + quote(property.columnName)));
				}
			});
		}
	}
		...
		if(!selectIsArray && !property.options.isAggregate || property.name == 'id' && !group || select && select.indexOf(property.name) >= 0) {
			if(property.isAllowed()) {
				...
				if(property.options.sqlish && typeof property.options.sqlish == 'string') {
					...
					selectQuery = selectQuery.column(self.knex.raw(sqlish).wrap('(', ') as ' + quote(property.columnName)));
				}
			}
		}
	});
...
Table.prototype.addValueClause = function(query, columnName, value, i, property) {
		...
		if(property && property.options.rawWhere) {
			...
			Object.keys(whereValue).forEach(function(key) {
				...
				if(key == '$or') {
					...
					query = query.whereNotIn(columnName, whereValue[key].map(function(value) {
	...
	if(Array.isArray(value)) {
		query = query.whereIn(columnName, value.map(function(whereValue) {
				...
				ors.forEach(function(or, index) {
					...
					this[whereType](function() {
						...
						propertyNames2.forEach(function(propertyName2) {
							...
							query = self.addValueClause(this, columnName, whereValue, 0, property);
						}, this);
					});
				}, this);
			});
			...
			if(firstProperty) {
				query.whereExists(function() {
					...
					if(firstProperty.isManyToMany()) {
						...
						subquery = subquery.whereRaw(otherProperty.columnName + ' = "' + self.name + '"."id"');
					}
					else {
