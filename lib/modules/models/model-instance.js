...
ModelInstance.prototype.addAssociation = function(property, modelInstance) {
	if(property.options.belongsTo) {
		...
		if(this._associations[property.name].length > 0 && this._associations[property.name][this._associations[property.name].length - 1].id === modelInstance.id) {
			// We do not want any duplicate instances...
		}
		else {
