 ...
 * @constructor
 */
function ModelInstance(model, row, changes, id, hookName, isPartial, isShallow) {
	this._model = model;
	this._map	= null;
	this._id = id;
...
ModelInstance.prototype.toJSON = function() {
	...
	Object.keys(properties).forEach(function(propertyName) {
		...
		if(property.options.isPrivate) {
			...
			if(!self._isPartial && property.isAssociation() && (property.options.autoFetch || self._associations[property.name])) {
...
ModelInstance.prototype.changePassword = function(currentPassword, newPassword, confirmPassword) {
	if(!this._model.isAuthenticator()) {
		throw new Error('ModelInstance#changePassword is only available on the authenticator model.');
	}
...
ModelInstance.prototype.addAssociation = function(property, modelInstance) {
	if(property.options.belongsTo) {
		...
		if(!this._associationsMaps[property.name]) {
		...
		if(modelInstance.id && !this._associationsMaps[property.name][modelInstance.id] || !modelInstance.id) {
			if(modelInstance.id) {
				this._associationsMaps[property.name][modelInstance.id] = true;
			}
...
ModelInstance.prototype.callHooks = function(hookNames) {
	...
	hookNames.forEach(function(hookName) {
		if(self._model[hookName]) {
			if(!result) {
				result = Q.when(self._model.models.app.injector.call(self._model[hookName], {self: self}, self));
			}
			else {
				result = result.then(function() {
					return Q.when(self._model.models.app.injector.call(self._model[hookName], {self: self}, self));
				});
			}
		}
	});
