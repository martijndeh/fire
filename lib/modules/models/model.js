 ...
 *
 * This method is only available on the front-end. This method is not available on the back-end.
 *
 * @name Model#authorize
 * @param {Dictionary} authenticateMap Dictionary containing the authenticating property and the password.
 * @function
 */
Model.prototype.authorize = function(authenticateMap, request) {
	if(!request) {
		throw new Error('`' + this._name + '#authorize` requires a request argument on the back-end.');
	}

	if(Object.keys(authenticateMap).length != 2 || !authenticateMap.password || typeof authenticateMap.password != 'string' || !authenticateMap[this.options.authenticatingProperty.name] || typeof authenticateMap[this.options.authenticatingProperty.name] != 'string') {
		var error = new Error('Bad Request');
		error.status = 400;
		throw error;
	}

	return this.getOne(authenticateMap)
		.then(function(instance) {
			request.session.at = instance.accessToken;
			return instance;
		});
};
 ...
 */
Model.prototype.signOut = function(request) {
	if(!request) {
		throw new Error('`' + this._name + '#signOut` requires a request argument on the back-end.');
	}
...
Model.prototype.findMe = function(request) {
	if(!request) {
		throw new Error('`' + this._name + '#getMe` requires a request argument on the back-end.');
	}
	...
	if(authenticatorModel != this) {
		throw new Error('Warning: `' + this._name + '#getMe` is not available because it\'s not the authenticator.');
	}
...
Model.prototype.update = function(whereMap, setMap, optionsMap) {
	...
	return Q.when(this._transformSetMap(setMap || {}, false))
		...
		.then(function(rows) {
			return self._createModelInstances(rows, 'afterUpdate');
		});
};
...
Model.prototype.remove = function(whereMap, optionsMap) {
	...
	return this._table.remove(where, options.limit, options.skip, options.orderBy)
		.then(function(rows) {
			return self._createModelInstances(rows);
		});
};
...
Model.prototype._transformSetMap = function(setMap, isNew) {
	...
	Object.keys(properties).forEach(function(propertyName) {
		...
		if(property.options.defaultValue && (!setMap[property.name] && isNew || property.options.defaultChangePropertyName && setMap[property.options.defaultChangePropertyName])) {
			result = result.then(function() {
 ...
 */
Model.prototype._createModelInstances = function(rows, hookName) {
