...
Model.prototype.authorize = function(authenticateMap) {
	if(!this.isAuthenticator()) {
		throw new Error('Model#authorize is only available on the authenticator model.');
	}
	...
	return this.getOne(whereMap).then(function(modelInstance) {
		return modelInstance.validateHash('password', authenticateMap.password)
			.then(function(valid) {
				if(valid) {
					return modelInstance.removePasswordReset()
						.then(function() {
...
Model.prototype.forgotPassword = function(authenticatingPropertyValue) {
	if(!this.isAuthenticator()) {
		throw new Error('Model#forgotPassword is only available on the authenticator model.');
	}
 ...
 */
Model.prototype._createModelInstances = function(rows, hookName, isPartial, isShallow) {
	...
	rows.forEach(function(row) {
		if(!instance || !instance.consumeRow(row)) {
			instance = new ModelInstance(self, row, null, row.id, hookName, isPartial, isShallow);
...
Model.prototype.create = function(fields) {
	if(Array.isArray(fields)) {
		return Q.all(fields.map(function(createMap) {
			return (new ModelInstance(this, null, createMap)).save();
		}, this));
	}
	else {
