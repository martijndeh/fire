 ...
 * @constructor
 */
function Model(name, models, activeMigration) {
	this._name = name;
	this.models = models;
	this.options = {};
 ...
 *
 * Currently this only updates one model instance.
 *
...
/**
 * This is called by a model instance to invoke an update.
 *
 * @access private
 *
 ...
 * @param  {String[]} optionsMap.select The properties to select. By default all properties are selected.
 *
 * This is useful if you want to limit the number of properties to fetch. If the property is an non-auto fetched association, the association is not fetched automatically. To fetch non-auto fetched associations, use the `optionsMap.associations` key.
 *
 * The `id` property is always returned.
 *
 * To limit the properties to only `name` of the `User` model, see the example below.
 * ```
...
Model.prototype.find = function(whereMap, optionsMap) {
	...
	var self = this;
	return this._table.select(where, options.limit, options.skip, options.orderBy, options.select)
		.then(function(result) {
