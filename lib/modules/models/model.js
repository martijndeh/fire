...
Model.prototype.getTableName = function() {
	return inflection.tableize(this.getName());
};
...
Model.prototype.addProperty = function(property, isNew) {
	if(!property.name || (property.name[0] == '_' || property.name[0] == '$') && !property.options.isReserved) {
...
/**
 * Adds a universal method to the model. This creates both instance methods and class methods to the model and the methods are available in both the back-end and front-end.
 *
 * To create a method:
 * ```
 * function User() {
 ...
 * ```
 *
 * See {@link Model#find} for additional information. This method is not universal and is only available on the back-end.
 */
...
Model.prototype.isCreated = function() {
	...
	return this._table.isCreated();
};

/**
 * Returns true if the model instance exists, or false otherwise. See {@link Model#count}.
 *
 * @param  {Dictionary} whereMap The where clause to add. See {@link Model#find}.
 * @return {Promise}          Boolean.
 */
Model.prototype.exists = function(whereMap) {
	if(this._activeMigration) {
		var args = new Array(arguments.length);
		for(var i = 0; i < args.length; ++i) {
			args[i] = arguments[i];
 ...
 *
 * @param  {String} propertyName The property to count. Optional.
 * @param  {Dictionary} whereMap     The where map. Optional. See {@link Model#find}.
 * @return {Promise}              Integer.
 */
Model.prototype.count = function(propertyName_, whereMap_) {
	if(this._activeMigration) {
		var args = new Array(arguments.length);
		for(var i = 0; i < args.length; ++i) {
			args[i] = arguments[i];
	...
	return this._table.count(propertyName, whereMap)
		.then(function(rows) {
			if(rows.length) {
 ...
 *
 * @param  {String} searchText      The search query e.g. "chicken" or "vegetable soup".
 * @param  {Dictionary} optionsMap
 * @param  {Dictionary} privateMap
 */
Model.prototype.search = function(searchText, whereMap, optionsMap, privateMap) {
	if(!this.searchConfig) {
		throw new Error('Model#search is not available. `searchConfig` is not configured on model `' + this._name + '`.');
	}
	...
	var parser = searchConfig.parser || function(text) {
	...
	var self = this;
	return this._table.search(parser(searchText), where, options.limit, options.skip, options.orderBy, options.groupBy, options.select, options.associations, options.autoFetchDepth)
		.then(function(rows) {
			return self._createModelInstances(rows, null, false, options.isShallow, privateMap);
		});
};
 ...
 *
 * A stream finds and returns all model instances matching `whereMap`. A stream finds model instances similar to {@link Model#find} and adds the model instances to the stream. But when new model instances get created, or updated, which match `whereMap` these new or updated model instances will be added to the stream.
 *
 * ```
 ...
 *
 * @param  {Dictionary} whereMap   The where map. See {@link Model#find}.
 * @param  {Dictionary} optionsMap The options to set e.g. limit. See {@link Model#find}.
 * @return {Object} A stream object which contains a `results` property containing all the model instances.
 */
Model.prototype.stream = function() {
	throw new Error('Model#stream is not implemented in the back-end yet. Model#stream is available in the front-end only.');
};
...
/**
 * Creates one or more model instance. This method is universal and available on the back-end and the front-end.
 *
...
Model.prototype.create = function(fields, privateMap) {
	if(Array.isArray(fields)) {
		return Q.all(fields.map(function(createMap) {
			var modelInstance = new ModelInstance(this, null, createMap, null, null, true, false, privateMap);
			return modelInstance.save();
		}, this));
	}
	else {
		var modelInstance = new ModelInstance(this, null, fields, null, null, true, false, privateMap);
		return modelInstance.save();
	}
};
...
Model.prototype._containsAutoFetchProperty = function() {
	var propertyNames = Object.keys(this._properties);
	for(var i = 0, il = propertyNames.length; i < il; i++) {
		var property = this._properties[propertyNames[i]];
