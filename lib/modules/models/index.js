...
Models.prototype._addModel = function(modelConstructor, name, force) {
	...
	var moduleProperties = null;
	if(this.app) {
		moduleProperties = this.app.moduleProperties;
	}
	else {
		// During migrations, we don't have an app thus can use the module properties mechanism.
...
Models.prototype._createModel = function(modelConstructor) {
	var self = this;
	var argumentNames = utils.getMethodArgumentNames(modelConstructor);
	return utils.ConstructWrapper(modelConstructor, argumentNames.map(function(argumentName) {
		var postfix = 'Model';

		if(argumentName.length < postfix.length || argumentName.lastIndexOf(postfix) != argumentName.length - postfix.length) {
			throw new Error('Invalid model named `' + argumentName + '` in `' + modelConstructor.name + '` dependency injection.');
		}

		var modelName = argumentName.substr(0, argumentName.length - postfix.length);
		var foundModel = self.findModel(modelName);
