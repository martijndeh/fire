...
Models.prototype.setup = function(basePath) {
	...
	return this.datastore.setup()
		...
		.then(function() {
			...
			if(basePath) {
				if(!self.app) {
					throw new Error('App is not available in Models#setup.');
				}
...
Models.prototype.postInstallModel = function(model) {
	...
	if(model.options.isThroughModel) {
		...
		//var property1 = throughModel.getProperty(utils.lcfirst(propertyNames[0]));
		//property.options.throughPropertyName = property1.name;

		//var property2 = throughModel.getProperty(utils.lcfirst(propertyNames[0]));
		//associatedProperty.options.throughPropertyName = property2.name;
		...
		if(!property.options.throughPropertyName) {
			throw new Error('Cannot find property to `' + property.model.getName() + '` on `' + model.getName() + '`.');
		}

		if(!associatedProperty.options.throughPropertyName) {
			throw new Error('Cannot find property to `' + associatedModel.getName() + '` on `' + model.getName() + '`.');
		}
	}
};
