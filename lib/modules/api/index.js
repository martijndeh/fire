 ...
 * The API module.
 *
 * This module generates model controllers of all models during build phase and executes the controllers in the run phase. The model controllers get generated to /.fire/api. To generate the model controllers:
 *
 * ```
 * $ grunt build
 * ```
...
/**
 * Loads the model controllers in the run phase. This method is called when the app starts up. The model controllers are loaded in the `.fire/api/` directory.
 *
 * @param  {String} basePath The root directory of the app.
 */
API.prototype.setup = function(basePath) {
	this.app.get('/api/*', function(request, response, next) {
	...
	this.app.requireDirSync(path.join(basePath, '.fire', 'api'));
};

API.prototype.stages = ['build', 'release', 'run'];
