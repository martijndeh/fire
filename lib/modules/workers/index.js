...
Workers.prototype.setup = function(basePath) {
	...
	debug('Workers#setup `' + process.env.AMQP_URL + '`');
	...
	if(!process.env.AMQP_URL) {
		debug('Not connecting to MQ.');

		if(Object.keys(this._constructors).length > 0) {
			throw new Error('Warning: workers exist but no AMQP_URL set.');
		}
	}
	else {
...
Workers.prototype.startConsuming = function(workerNames) {
	if(typeof workerNames == 'undefined' || !Array.isArray(workerNames)) {
		throw new Error('Specify an array of worker names in Workers#startConsuming.');
	}
