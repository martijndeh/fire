...
function BuildSystem(stageMethods, $q) {
    ...
    stageMethods.addMethod('build', function procfile(app) {
        ...
        var contents = null;
        try {
            contents = fs.readFileSync(savePath, {encoding: 'utf8'});
        }
        catch(e) {
            //
        }
    ...
    stageMethods.addMethod('build', function less(app) {
        ...
        var lessConfig = app.settings('less') || {};
        var lessPlugins = [];

        // User can choose to have autoprefixer or not, also can config their browser support
        if (lessConfig.autoprefixer !== false) {
            lessPlugins.push(new LessPluginAutoPrefix({browsers: lessConfig.autoprefixer && lessConfig.autoprefixer.browsers || ['> 5%', 'last 2 versions']}));
        }

        // Allow user to also add in additional plugins
        if (Array.isArray(lessConfig.plugins)) {
            lessPlugins = lessPlugins.concat(lessConfig.plugins);
        }
