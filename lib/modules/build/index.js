...
Build.prototype.extendConfig = function(gruntConfig, grunt) {
	...
	if(optionsApp.container.numberOfApps() == 1) {
		...
		gruntConfig.uglify = {
			...
			build: {
				src: '.fire/.build/public/scripts/fire.js',
				dest: '.fire/.build/public/scripts/fire.min.js'
			}
		};
	}
	else {
		...
		gruntConfig.uglify = {
			...
			build: {
				src: '.fire/.build/public/' + optionsApp.name + '/scripts/fire.js',
				dest: '.fire/.build/public/' + optionsApp.name + '/scripts/fire.min.js'
			}
		};
	}
	...
	grunt.registerTask('run', function() {
		...
		var command = './node_modules/fire/node_modules/foreman/nf.js';

		var foreman = spawn(command, ['start', '--procfile', optionsApp.container.numberOfApps() == 1 ? '.fire/.build/Procfile' : '.fire/.build/' + optionsApp.name + '/Procfile'], {stdio: 'inherit'});
			...
			Q.when(app.workers.setup(basePath))
				...
				.then(function() {
					if(app.container.numberOfApps() > 1) {
						return mkdir(path.join(basePath, '.fire', '.build', app.name));
					}
				})
				.then(function() {
					var processes = [];
					...
					var savePath;
					if(app.container.numberOfApps() > 1) {
						savePath = path.join(basePath, '.fire', '.build', app.name, 'Procfile');
					}
					else {
						savePath = path.join(basePath, '.fire', '.build', 'Procfile');
					}
