...
app.post('/api/user2s', function(app, response, request, User2Model, UserModel) {
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
			var accessControl = User2Model.getAccessControl();
			return Q.when(accessControl.canCreate(app, {authenticator: authenticator, request: request, response: response}))
				.then(function(canCreate) {
					if(canCreate === true) {
						...
						if(User2Model.options.automaticPropertyName) {
							if(createMap[User2Model.options.automaticPropertyName]) {
								var error = new Error('Cannot set automatic property manually.');
								error.status = 400;
								throw error;
							}

							createMap[User2Model.options.automaticPropertyName] = authenticator;
						}

						if(_canSetProperties(Object.keys(createMap), User2Model)) {
							return User2Model.create(createMap);
						}
						else {
...
app.get('/api/user2s', function(request, response, app,  User2Model, UserModel) {
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
			var accessControl = User2Model.getAccessControl();
			return Q.when(accessControl.canRead(app, {authenticator: authenticator, request: request, response: response}))
				.then(function(canRead) {
					if(canRead === true) {
						...
						if(User2Model.options.automaticPropertyName) {
							queryMap[User2Model.options.automaticPropertyName] = authenticator;
						}

						return User2Model.find(queryMap, optionsMap);
					}
					else {
						throw unauthenticatedError(authenticator);
					}
...
app.get('/api/user2s/:id', function(request, response, app,  User2Model, UserModel) {
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
			var accessControl = User2Model.getAccessControl();
		...
		.spread(function(canRead, authenticator) {
			if(canRead === true) {
				...
				if(User2Model.options.automaticPropertyName) {
					whereMap[User2Model.options.automaticPropertyName] = authenticator;
				}

				return User2Model.getOne(whereMap);
			}
			else {
				throw unauthenticatedError(authenticator);
			}
...
app.put('/api/user2s/:id', function(request, response, app,  User2Model, UserModel) {
	var accessControl = User2Model.getAccessControl();
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
		...
		.spread(function(canUpdate, authenticator) {
			if(canUpdate) {
				...
				var keyPath = accessControl.getPermissionKeyPath('update');
				if(keyPath) {
					if(!User2Model.getProperty(keyPath)) {
				...
				whereMap.id = request.param('id');
				return [_canUpdateProperties(Object.keys(request.body), User2Model), whereMap, authenticator];
			}
			else {
				throw unauthenticatedError(authenticator);
			}
		...
		.spread(function(canUpdateProperties, whereMap, authenticator) {
			if(canUpdateProperties) {
				return Q.all([User2Model.updateOne(whereMap, request.body), authenticator]);
			}
			else {
				var error = new Error();
				error.status = 400;
...
app.delete('/api/user2s', function(request, response, app,  User2Model, UserModel) {
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
			var accessControl = User2Model.getAccessControl();
			return Q.when(app.injector.call(accessControl.getPermissionFunction('delete'), {authenticator: authenticator, request: request, response: response}))
				.then(function(canDelete) {
					if(canDelete === true) {
						...
						return User2Model.remove(whereMap);
					}
					else {
						throw unauthenticatedError(authenticator);
					}
...
app.delete('/api/user2s/:id', function(request, response, app,  User2Model, UserModel) {
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
			var accessControl = User2Model.getAccessControl();
			return Q.when(app.injector.call(accessControl.getPermissionFunction('delete'), {authenticator: authenticator, request: request, response: response}))
			.then(function(canDelete) {
				if(canDelete === true) {
					...
					return User2Model.removeOne(whereMap);
				}
				else {
					throw unauthenticatedError(authenticator);
				}
