...
app.post('/api/user1s', function(app, response, request, User1Model, UserModel) {
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
			var accessControl = User1Model.getAccessControl();
			return Q.when(accessControl.canCreate({authenticator: authenticator, request: request, response: response}))
				.then(function(canCreate) {
					if(canCreate) {
						...
						if(User1Model.options.automaticPropertyName) {
							if(createMap[User1Model.options.automaticPropertyName]) {
								var error = new Error('Cannot set automatic property manually.');
								error.status = 400;
								throw error;
							}

							createMap[User1Model.options.automaticPropertyName] = authenticator;
						}

						if(_canSetProperties(Object.keys(createMap), User1Model)) {
							return User1Model.create(createMap);
						}
						else {
...
app.get('/api/user1s', function(request, response, app,  User1Model, UserModel) {
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
			var accessControl = User1Model.getAccessControl();
			return Q.when(accessControl.canRead({authenticator: authenticator, request: request, response: response}))
				.then(function(canRead) {
					if(canRead) {
						...
						if(User1Model.options.automaticPropertyName) {
							queryMap[User1Model.options.automaticPropertyName] = authenticator;
						}

						return User1Model.find(queryMap, optionsMap);
					}
					else {
						throw unauthenticatedError(authenticator);
					}
...
app.get('/api/user1s/:id', function(request, response, app,  User1Model, UserModel) {
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
			var accessControl = User1Model.getAccessControl();
		...
		.spread(function(canRead, authenticator) {
			if(canRead) {
				...
				if(User1Model.options.automaticPropertyName) {
					whereMap[User1Model.options.automaticPropertyName] = authenticator;
				}

				return User1Model.getOne(whereMap);
			}
			else {
				throw unauthenticatedError(authenticator);
			}
...
app.put('/api/user1s/:id', function(request, response, app,  User1Model, UserModel) {
	var accessControl = User1Model.getAccessControl();
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
		...
		.spread(function(canUpdate, authenticator) {
			if(canUpdate) {
				...
				whereMap.id = request.param('id');
				return [_canUpdateProperties(Object.keys(request.body), User1Model), whereMap, authenticator];
			}
			else {
				throw unauthenticatedError(authenticator);
			}
		...
		.spread(function(canUpdateProperties, whereMap, authenticator) {
			if(canUpdateProperties) {
				return Q.all([User1Model.updateOne(whereMap, request.body), authenticator]);
			}
			else {
				var error = new Error();
				error.status = 400;
...
app.delete('/api/user1s', function(request, response, app,  User1Model, UserModel) {
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
			var accessControl = User1Model.getAccessControl();
			return Q.when(accessControl.canDelete({authenticator: authenticator, request: request, response: response}))
				.then(function(canDelete) {
					if(canDelete) {
						...
						return User1Model.remove(whereMap);
					}
					else {
						throw unauthenticatedError(authenticator);
					}
...
app.delete('/api/user1s/:id', function(request, response, app,  User1Model, UserModel) {
	return findAuthenticator(UserModel, request)
		.then(function(authenticator) {
			var accessControl = User1Model.getAccessControl();
			return Q.when(accessControl.canDelete({authenticator: authenticator, request: request, response: response}))
			.then(function(canDelete) {
				if(canDelete) {
					...
					return User1Model.removeOne(whereMap);
				}
				else {
					throw unauthenticatedError(authenticator);
				}
