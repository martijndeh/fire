...
app.factory('UserInApp1Model', ['$http', '$q', 'FireModel', '$injector', '$route', '$routeParams', '$location', function($http, $q, FireModel, $injector, $route, $routeParams, $location) {
    var model = new FireModel('UserInApp1', [], '/api/user-in-app1s');
...
app.factory('FireModelInstanceUserInApp1', ['UserInApp1Model', '$q', '$http', '$injector', function(UserInApp1Model, $q, $http, $injector) {
    return function(setMap, path, shouldBeUndefined) {
        if(shouldBeUndefined) {
            throw new Error('FireModelInstanceUserInApp1 only accepts two arguments now.');
        }
        ...
        if(this._map.id) {
            this._endpoint = UserInApp1Model.endpoint + '/' + this._map.id;
        }
        else {
            this._endpoint = null;
        }
        ...
        this.remove = function() {
        	return UserInApp1Model.remove(this._map.id);
        };

        this.save = function() {
            if(this._map === null) {
                return UserInApp1Model.create(this._changes)
                    .then(function(modelInstance) {
                        return self.refresh(modelInstance);
                    });
            }
                ...
                if(numberOfChanges) {
                    ...
                    return UserInApp1Model._put(this._endpoint, queryMap)
                        .then(function(instance) {
                            self._changes = {};
