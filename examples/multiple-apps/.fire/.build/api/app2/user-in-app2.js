...
app.post('/api/user-in-app2s', function(app, response, request, UserInApp2Model) {
	return findAuthenticator(null, request)
		.then(function(authenticator) {
			var accessControl = UserInApp2Model.getAccessControl();
				...
				.then(function(canCreate) {
					if(canCreate) {
						var checkCreateMap = function(createMap) {
							...
							if(UserInApp2Model.options.automaticPropertyName) {
								createMap[UserInApp2Model.options.automaticPropertyName] = authenticator;
							}



							if(_canSetProperties(Object.keys(createMap), UserInApp2Model)) {
								return createMap;
							}
							else {
						...
						if(Array.isArray(request.body)) {
							...
							return UserInApp2Model.create(createMaps, {authenticator: authenticator, request: request, response: response});
							
						}
						else {
							return UserInApp2Model.create(checkCreateMap(request.body || {}), {authenticator: authenticator, request: request, response: response});
						}
					}
					else {
...
app.get('/api/user-in-app2s', function(request, response, app,  UserInApp2Model) {
	return findAuthenticator(null, request)
		.then(function(authenticator) {
			...
			if(UserInApp2Model.options.automaticPropertyName) {
				whereMap[UserInApp2Model.options.automaticPropertyName] = authenticator;
			}
			...
			return Q.when(accessControl.canRead({authenticator: authenticator, request: request, response: response, whereMap: whereMap}))
				.then(function(canRead) {
					if(canRead) {
						...
						return UserInApp2Model.find(whereMap, optionsMap);
					}
					else {
						throw unauthenticatedError(authenticator);
					}
...
app.get('/api/user-in-app2s/:id', function(request, response, app,  UserInApp2Model) {
	return findAuthenticator(null, request)
		.then(function(authenticator) {
			var whereMap = request.query || {};
			...
			return Q.when(accessControl.canRead({authenticator: authenticator, request: request, response: response, whereMap: whereMap}))
				.then(function(canRead) {
					if(canRead) {
						...
						return UserInApp2Model.getOne(whereMap, optionsMap);
					}
					else {
						throw unauthenticatedError(authenticator);
					}
...
app.put('/api/user-in-app2s/:id', function(request, response, app,  UserInApp2Model) {
	var accessControl = UserInApp2Model.getAccessControl();
	return findAuthenticator(null, request)
		.then(function(authenticator) {
			var whereMap = request.query || {};
			...
			return Q.when(accessControl.canUpdate({authenticator: authenticator, request: request, response: response, whereMap: whereMap}))
				.then(function(canUpdate) {
					if(canUpdate) {
						...
						return [_canUpdateProperties(Object.keys(request.body), UserInApp2Model), whereMap, authenticator];
					}
					else {
						throw unauthenticatedError(authenticator);
					}
		...
		.spread(function(canUpdateProperties, whereMap, authenticator) {
			if(canUpdateProperties) {
				return Q.all([UserInApp2Model.updateOne(whereMap, request.body), authenticator]);
			}
			else {
				throw badRequestError();
			}
...
app.put('/api/user-in-app2s', function(request, response, app,  UserInApp2Model) {
	return findAuthenticator(null, request)
		.then(function(authenticator) {
			var whereMap = request.query || {};
			...
			return Q.when(accessControl.canUpdate({authenticator: authenticator, request: request, response: response, whereMap: whereMap}))
				.then(function(canUpdate) {
					if(canUpdate) {
						return Q.when(_canUpdateProperties(Object.keys(request.body || {}), UserInApp2Model))
							.then(function(canUpdateProperties) {
								if(canUpdateProperties) {
									...
									return UserInApp2Model.update(whereMap, request.body || {});
								}
								else {
									throw badRequestError();
								}
...
app.delete('/api/user-in-app2s', function(request, response, app,  UserInApp2Model) {
	return findAuthenticator(null, request)
		.then(function(authenticator) {
			var whereMap = request.query || {};
			if(UserInApp2Model.options.automaticPropertyName) {
			...
			return Q.when(accessControl.canDelete({authenticator: authenticator, request: request, response: response, whereMap: whereMap}))
				.then(function(canDelete) {
					if(canDelete) {
						...
						return UserInApp2Model.remove(whereMap, optionsMap);
					}
					else {
						throw unauthenticatedError(authenticator);
					}
...
app.delete('/api/user-in-app2s/:id', function(request, response, app,  UserInApp2Model) {
	return findAuthenticator(null, request)
		.then(function(authenticator) {
			var whereMap = request.query || {};
			whereMap.id = request.params.id;
			...
			.then(function(canDelete) {
				if(canDelete) {
					...
					return UserInApp2Model.removeOne(whereMap, optionsMap);
				}
				else {
					throw unauthenticatedError(authenticator);
				}
