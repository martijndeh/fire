				...
				.then(function(canCreate) {
					if(canCreate) {
						...
						if(Array.isArray(request.body)) {
							...
							return UserInApp1Model.create(createMaps, {authenticator: authenticator, request: request, response: response});
							
						}
						else {
							return UserInApp1Model.create(checkCreateMap(request.body || {}), {authenticator: authenticator, request: request, response: response});
						}
					}
					else {
			...
			return Q.when(accessControl.canRead({authenticator: authenticator, request: request, response: response, whereMap: whereMap}))
				.then(function(canRead) {
					if(canRead) {
						...
						return UserInApp1Model.find(whereMap, optionsMap);
					}
					else {
						throw unauthenticatedError(authenticator);
					}
			...
			return Q.when(accessControl.canRead({authenticator: authenticator, request: request, response: response, whereMap: whereMap}))
				.then(function(canRead) {
					if(canRead) {
						...
						return UserInApp1Model.getOne(whereMap, optionsMap);
					}
					else {
						throw unauthenticatedError(authenticator);
					}
...
app.put('/api/user-in-app1s', function(request, response, app,  UserInApp1Model) {
	return findAuthenticator(null, request)
		.then(function(authenticator) {
			var whereMap = request.query || {};
			...
			return Q.when(accessControl.canUpdate({authenticator: authenticator, request: request, response: response, whereMap: whereMap}))
				.then(function(canUpdate) {
					if(canUpdate) {
						return Q.when(_canUpdateProperties(Object.keys(request.body || {}), UserInApp1Model))
							.then(function(canUpdateProperties) {
								if(canUpdateProperties) {
									...
									return UserInApp1Model.update(whereMap, request.body || {});
								}
								else {
									throw badRequestError();
								}
			...
			return Q.when(accessControl.canDelete({authenticator: authenticator, request: request, response: response, whereMap: whereMap}))
				.then(function(canDelete) {
					if(canDelete) {
						...
						return UserInApp1Model.remove(whereMap, optionsMap);
					}
					else {
						throw unauthenticatedError(authenticator);
					}
		...
		.then(function(authenticator) {
			...
			.then(function(canDelete) {
				if(canDelete) {
					...
					return UserInApp1Model.removeOne(whereMap, optionsMap);
				}
				else {
					throw unauthenticatedError(authenticator);
				}
