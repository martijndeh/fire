...
function FireModelInstanceUserInApp1(setMap, model, path) {
	var self = this;
...
FireModelInstanceUserInApp1.prototype = new FireModelInstance();



function FireModelUserInApp1($http, $q, models) {
	FireModel.call(this, $http, $q, models);

	this.endpoint = '/api/user-in-app1s';
}
FireModelUserInApp1.prototype = new FireModel();

FireModelUserInApp1.prototype.parseResult = function(setMapOrList, path) {
	if(Object.prototype.toString.call(setMapOrList) === '[object Array]') {
		var self = this;
		return setMapOrList.map(function(setMap) {
			return new FireModelInstanceUserInApp1(setMap, self, path);
		});
	}
	else {
		return new FireModelInstanceUserInApp1(setMapOrList, this, path);
	}
};



app.factory('UserInApp1Model', ['$http', '$q', 'FireModels', function($http, $q, FireModels) {
	return new FireModelUserInApp1($http, $q, FireModels);
}]);
...
app.service('FireModels', ['$http', '$q', function($http, $q) {
	...
	this.UserInApp1 = new FireModelUserInApp1($http, $q, this);
